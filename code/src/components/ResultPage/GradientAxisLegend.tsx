...
const GradientAxisLegend: React.FC = () => {
    return (
        <div className="flex items-center h-full">
            <div className="flex flex-col items-center justify-center h-full bg-white/50 dark:bg-gray-800/50 backdrop-blur-md rounded-full py-4 px-4 shadow-lg">
                <div className="relative w-4 h-48 bg-gradient-to-t from-transparent via-blue-400 to-red-500 rounded-full">
                    ...
                    {[0, 25, 50, 75, 100].map(val => (
                        <div key={val} className="absolute w-full left-0" style={{ bottom: `${val}%` }}>
                            ...
                            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-xs font-bold text-gray-600 dark:text-gray-300">
                                {val}%
                            </span>
                        </div>
                    ))}
