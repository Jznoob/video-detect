...
const getRiskLevel = (p: number) => {
  if (p > 0.9) return { label: '高', color: 'bg-red-500' };
...
const DetectionSummaryCard: React.FC<DetectionSummaryCardProps> = ({
  ...
  return (
    <div className="bg-white/30 dark:bg-gray-800/30 rounded-lg shadow p-6">
      <div className="flex items-center gap-3 mb-2">
        <span className={`px-3 py-1 rounded-full text-white text-sm font-bold ${risk.color}`}>风险：{risk.label}</span>
        <span className="text-gray-700 dark:text-gray-200 font-semibold">伪造概率: {(forgedProbability * 100).toFixed(2)}%</span>
      </div>
