...
const Dashboard: React.FC = () => {
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 md:px-8 py-8 space-y-10">
      ...
      {/* 统计卡片区或骨架 */}
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-gray-800 dark:bg-gray-800">
        {isLoading
          ? Array.from({ length: 3 }).map((_, i) => (
              <div
                key={i}
                className="h-20 w-full min-w-[160px] max-w-[320px] bg-black dark:bg-gray-700 rounded-xl animate-pulse"
                style={{ height: 80 }}
              />
            ))
          : stats.map((item) => (
      ...
      {/* 最近任务区或骨架 */}
      <section className="bg-gray-800 dark:bg-gray-800 rounded-xl shadow px-6 py-5">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-2 bg-gray-800 dark:bg-gray-800 rounded-xl shadow p-6">
          <h2 className="text-lg font-semibold text-white dark:text-white mb-4">
            检测类型占比
          </h2>
          <DetectionTypeChart />
        </div>
        <div className="lg:col-span-3 bg-gray-800 dark:bg-gray-800 rounded-xl shadow p-6">
          <h2 className="text-lg font-semibold text-white dark:text-white mb-4">
            平均检测时长 (近7日)
          </h2>
